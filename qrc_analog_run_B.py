...
def log_moments(opt_state, case_num, time_steps,epoch):
    ...
    Return {block: (‖m‖₂, ‖v‖₂)}.
    Supports:
        • case 1-4 : three masked branches (t, h, J)
        • case 5   : T masked Adam branches where each time step has its own moment
        • case 6-7 : one global Adam
    """
    
    if case_num in [1,2,3,4]:
        # opt_state = (clip, Adam_τ, Adam_h, Adam_J)
...
def get_groupwise_lr_trees(
    ...
    * Wang & Kolter, *ICML* (2023) “RMS-trust ratio” – motivation for step 2.
    ...
    lr_tau_raw,lr_tau_max = eta_max(tgt_tau,g_tau)
    lr_h_raw, lr_h_max = eta_max(tgt_h,g_h)
    lr_J_raw, lr_J_max = eta_max(tgt_J,g_J)
    ...
    r_tau_pre, r_h_pre, r_J_pre = r_scale(g_tau), r_scale(g_h), r_scale(g_J)
    ...
    lr_tau = per_param_lr(lr_tau_max, r_tau, g_tau)
    lr_h   = per_param_lr(lr_h_max,   r_h,   g_h)
    lr_J   = per_param_lr(lr_J_max,   r_J,   g_J)
    ...
    if debug:
        ...
        print(f"  τ: {lr_tau_raw:.2e} → {lr_tau_max:.2e}")
        print(f"  h: {lr_h_raw:.2e} → {lr_h_max:.2e}")
        print(f"  J: {lr_J_raw:.2e} → {lr_J_max:.2e}")
...
if __name__ == '__main__':
    ...
    t_bound = bound_dict[t_bound_key]
    ...
    if case_num in [2,3]:
        ...
        folder = f'./analog_results_B/trainsize_{N_train+add}_epoch{num_epochs}/case_{case_num}/tgt_granular_{per_block_target_update}/t={t_bound_key}/scale={scale_by_num_train}/'
        ...
        for time_steps in trots:
            ...
            for N_reserv in res:
                ...
                for num_bath,bath in zip(num_baths,baths):
                    ...
                    main_params = jax.random.uniform(
                        params_key, 
                    ...
                    init_params_dict = {'K_coef': jnp.asarray(K_coeffs)}
