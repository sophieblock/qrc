...
def run_experiment(params, bath_params, steps, n_rsv_qubits, n_ctrl_qubits, K_coeffs, trotter_steps, static, gate, gate_name, folder, test_size, training_size, opt_lr,dataset_key):
    ...
    # )
    # opt = optax.chain(
    #     optax.clip_by_global_norm(1.0),adam, reduce_on_plateau)
...
# --------------------------------------------------------------------------------------------
    ...
    test_size = 2000
    bath = False
    folder = f'./gate_results_trainable_global/trainsize_{training_size}_epoch{steps}_per_param_costcut_1e-10/'
    for gate_idx,gate in enumerate(gates):
        # if gate_idx not in [6]:
        # if gate_idx not in [7]:
        #     continue
