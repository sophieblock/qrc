...
def improved_get_initial_lr_per_param(grads, base_step=0.005, min_lr=1e-4, max_lr=0.2,
    ...
    Method:
      1. Compute |grad| for each parameter across the PyTree.
      2. Concatenate all absolute values into one vector (all_abs).
      3. Optionally clip extreme values in all_abs (using outlier_clip) to obtain clipped_abs.
      4. Compute the robust scale “r”:
             median_val = median(clipped_abs)
             MAD = median(|clipped_abs - median_val|)
             r = median_val + MAD
         (If r is zero, fallback to median_val.)
      5. Define a fudge factor:
             fudge = (if fudge_factor is provided, use it; otherwise, set fudge = 0.1 * r)
      6. For each parameter (with absolute value |g|), compute raw learning rate:
             lr_raw = base_step * (r / (|g| + fudge))
      7. Clamp lr_raw to [min_lr, max_lr] for numerical safety.
      8. Restore the original PyTree structure.
    
    This procedure ensures that:
      - Parameters whose gradient magnitude is much below the typical scale r receive a higher lr.
      - Parameters with high gradients get a lower lr.
      - The fudge term is derived from the data (r) rather than using max_lr.
    
    Arguments:
      grads        : A PyTree of gradients (e.g., nested dicts/lists of jax.numpy arrays).
      base_step    : Baseline multiplier for the learning rate (if |grad| ≈ r, then lr ≈ base_step).
      min_lr       : Minimum allowed learning rate.
      max_lr       : Maximum allowed learning rate.
      outlier_clip : Quantile (between 0 and 1) used to clip extreme absolute gradient values
                     before computing robust statistics.
      fudge_factor : If provided, use this as the fudge term; otherwise, it is set to 0.1*r.
    
    Returns:
      A PyTree of learning rates having the same structure as grads.
    """
    ...
    abs_leaves = [jnp.abs(g) + 1e-12 for g in grad_leaves]  # add a tiny epsilon to avoid zero
    # Concatenate all absolute values.
    all_abs = jnp.concatenate([jnp.ravel(g) for g in abs_leaves])
    
    # Step 3: Optionally clip extreme values for robust statistics.
    ...
    # For each parameter with gradient g, lr_raw = base_step * (r / (|g| + fudge))
    lr_leaves = [base_step * (r / (jnp.abs(g) + fudge)) for g in grads]
