...
from .quantum_gates import QuantumGate, SWAP

# import workflow.simulation.refactor.quantum_gates as quantum_gateset
from .utilities import adjacent_edges_from_edge, adjacent_edge_idxs_from_edge

# import qiskit.circuit.quantumcircuit as QiskitQuantumCircuit
from qiskit import QuantumCircuit as QiskitQuantumCircuit
import qiskit.circuit.library as qiskit_library
import workflow.simulation.refactor.quantum_gates as quantum_gateset
from workflow.simulation.refactor.data_types import QAny
...
class QuantumCircuit:
    def __init__(self, qubit_count=None, gate_set=None, instructions=None):
        ...
        # assert all(
        #     gate.__class__.__bases__[0] ==  QuantumGate for gate in self.gate_set
        # ), f"One or more elements in {gate_set} is not a QuantumGate object"
...
def qiskit_to_quantum_circuit(qc: QiskitQuantumCircuit):
...
def quantum_circuit_to_qiskit(qc: QuantumCircuit):
    circuit = QiskitQuantumCircuit(qc.qubit_count, 0)
...
class LayoutSynthesizer:
    ...
    def compute_circuit_depth(self, quantum_circuit: QuantumCircuit):
        ...
        # depth_at_idx = [0] * quantum_circuit.qubit_count
        # for instruction in quantum_circuit.instructions:
        #     new_depth = max(
        #         depth_at_idx[gate_idx] for gate_idx in instruction.gate_indices
        #     )
    ...
    def find_optimal_layout(self):
        ...
        """
        self.solve()
        result_circuit, initial_qubit_map, final_qubit_map, objective_result = (
            self.post_process()
        )
        return result_circuit, initial_qubit_map, final_qubit_map, objective_result
    ...
    def solve(self):
        ...
        print(f"Layout synthesis time: {datetime.datetime.now() - start_time}")
    ...
    def post_process(self):
        ...
        # final_qubit_map = [
        #     model[pi[qubit_idx][self.results["depth"]-1]].as_long()
        #     for qubit_idx in range(self.circuit.qubit_count)
        # ]
        objective_result = self.results["depth"]

        print(f"Initial qubit mapping: {initial_qubit_map}")
        print(f"Final qubit mapping: {final_qubit_map}")
...
class LayoutSynthesizer:
    ...
    def solve(self):
        ...
        start_time = datetime.datetime.now()
        while not found_solution:
            logger.debug(f"Attempting maximal depth {self.circuit_depth}...")
            self.initialize_variables()
            self.add_constraints()
            self.add_optimization_objective()
        ...
        logger.debug(f"Layout synthesis time: {datetime.datetime.now() - start_time}")
