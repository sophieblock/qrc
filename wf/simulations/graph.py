...
class DirectedEdge:
    ...
    def __init__(
        ...
        # logger.debug(f'{self} inittialized with data: {self.data}')
...
class Node:
    ...
    def start(self):
        ...
        map correctly.
        """
        # logger.debug(f'Attempting to start {self}')
    ...
    def __initialize_process_with_inputs(self):
        ...
        # this is where we showcase how helpful the CompositeMod/parameterized Process is (we shouldnt have to 
        # re-traverse the sub processes since we traced the process
        # del self.model
    ...
    def complete(self):
        logger.debug(f"Completing {self}")
        assert (
    ...
    def generate_output_edges(self, include_thru=True):
        """
        Generates the output edges of a Node after it's Process has completed execution,
        including new edges for the right registers (the 'generate_output()' data)
        and optionally the left/thru registers. Now with extra debug logs!
        """
        logger.debug(f"     [EdgeGen] Starting generate_output_edges for Node {repr(self)} (include_thru={include_thru})")
        # logger.debug(f"[EdgeGen] Node signature: {self.process.signature}")
        # logger.debug(f"[EdgeGen] normalized_map keys: {list(self.process.normalized_map.keys())}")
        # logger.debug(f"[EdgeGen] expected_input_properties: {getattr(self.process, 'expected_input_properties', None)}")

        right_output_ids = []
        data_id_to_spec = {}

        # 1) For each data object from self.process.generate_output(), create an OUTPUT edge
        for result in self.process.generate_output():
            ...
            # The name of the signature register might come from the usage/hint
            # or from your ephemeral rename logic. If there's a mismatch, you'll see it here.
            result_meta_hint = result.metadata.hint
            try:
                outspec = self.process.signature.get_right(name=result_meta_hint)
                logger.debug(f"     [EdgeGen]   Matched result hint '{result_meta_hint}' to signature register => {outspec}")
            except KeyError:
                logger.debug(f"     [EdgeGen]   Could not find a RIGHT register with name={result_meta_hint}")
                outspec = None

            right_output_ids.append(result.id)
            data_id_to_spec[result.id] = outspec

            # Create the actual OUTPUT edge with no dest_nodes (yet).
            edge = DirectedEdge(
                data=(result,),
                source_node=self,
                dest_nodes=None,
            ...
            # logger.debug(f"[EdgeGen]  Created edge => {edge} with data usage={usage_str}")
            self.output_edges.append(edge)

        # 2) If we also want to re-emit THRU data, look at the left-flow registers
        if include_thru:
            # logger.debug("  [EdgeGen]  Mapping left/thru registers for output edges now")
            left_specs = [s for s in self.process.signature if s.flow & Flow.LEFT]
            # logger.debug(f" [EdgeGen]  left_specs => {left_specs}")
            assert left_specs == list(self.process.signature.lefts()), "Debug check left specs"

            # For each left spec, we retrieve the data from self.process.normalized_map.
            # This is usually ephemeral vs usage-based naming. The logs show if there's a mismatch.
            for thru_reg in left_specs:
                # logger.debug(f" [EdgeGen]   Checking THRU register => {thru_reg}")
                # e.g. if ephemeral name was renamed from 'arg0' => 'Matrix'
                # you must confirm the new name is the one in normalized_map
                if thru_reg.name not in self.process.normalized_map:
                    logger.debug(f"     Mismatch: {thru_reg.name} not in normalized_map => {self.process.normalized_map.keys()}")
                    continue

                thru_data = self.process.normalized_map[thru_reg.name]
                logger.debug(f"     Found THRU data => {thru_data}, usage={thru_data.properties.get('Usage')}")

                # If the data was also output before, skip it or raise an error
                if thru_data.id in right_output_ids:
                    logger.debug(f"     Data already output => {thru_data.id}, skipping")
                    continue

                # else we create an OUTPUT edge for it
                edge = DirectedEdge(
                    data=(thru_data,),
                    source_node=self,
                    dest_nodes=None,
                ...
                logger.debug(f"     Created THRU edge => {edge}")
                self.output_edges.append(edge)

        logger.debug(f" [EdgeGen]  final output_edges => {self.output_edges}")
    def map_edges_to_output_nodes(self):
        """
        Maps outgoing DirectedEdges to the next Node(s) if usage / data property matches.
        We'll add debug lines to see what's being matched or not matched.
        """
        if self.output_nodes == []:
            logger.debug(f" [Hooking] Node {self.id} has no output_nodes, skipping hooking.")
            return

        logger.debug(f" [Hooking] {repr(self)} has {len(self.input_edges)} input edges, "
                    f"{len(self.output_nodes)} out nodes, {len(self.output_edges)} output edges")

        for edge in self.output_edges:
            if edge.edge_type == DirectedEdge.OUTPUT:
                # logger.debug(f" [Hooking] Considering {edge} for hooking with {self.output_nodes}")
                try:
                    expected_nodes = self.get_expected_nodes(edge, self.output_nodes)
                    # logger.debug(f" [Hooking] found matching nodes => {expected_nodes}")
                except ExpectedNodeError:
                    # logger.debug(f" [Hooking] No matching node for {edge}, skipping.")
                    continue
    ...
    def __edge_properties_in_output_node(self, data_properties: dict, output_node) -> bool:
        """
        Checks whether data_properties of a DirectedEdge are contained in
        the expected_input_properties of output_node. Instead of directly comparing
        the raw dictionaries, we compare the sanitized 'Usage' values if available.
        """
        from workflow.simulation.refactor.register import _sanitize_name  # import the sanitizer

        # If 'Usage' is present, use it to perform the comparison.
        data_usage = data_properties.get("Usage", "").strip()
        if data_usage:
            sanitized_data_usage = _sanitize_name(data_usage)
            for input_properties in output_node.expected_input_properties:
                expected_usage = input_properties.get("Usage", "").strip()
                if expected_usage:
                    sanitized_expected = _sanitize_name(expected_usage)
                    if sanitized_data_usage == sanitized_expected:
                        return True
            # If none of the expected usages match, return False.
            return False
        else:
            # Fallback: if no 'Usage' is provided, perform the original comprehensive check.
            for input_properties in output_node.expected_input_properties:
                if all(
    ...
    #         Node: The expected Node that DirectedEdge edge connects to
    #     """
    #     logger.debug(f"DirectedEdge {edge} with data {edge.data} to be connected with {output_nodes}")
    #     assert all(
    #         isinstance(output_node, Node) for output_node in output_nodes
    #     ), f"{output_nodes} must be a list of Nodes"
    #     assert (
...
class Network:
        ...
        while len(remaining_nodes) != 0 or len(active_nodes) != 0:
            ...
            while node_idx < len(remaining_nodes):
                ...
                try:
                    ...
                    node.process.update()
                    if not simulate:
                        node.process.time_till_completion = node.process.time_till_completion
                    ...
                    logger.debug(f'Allocation or init error')
                    node_idx = node_idx + 1
                    continue
            ...
            logger.debug(f'active_nodes: {active_nodes}')
            ...
            while node_idx < len(active_nodes):
                ...
                node_idx = node_idx + 1
        self.status = Network.INACTIVE
        # logger.debug(f'Finished running {self}. Total time: {results.keys()}')
    ...
    def __allocate_resources(self, node: Node):
        ...
        # node.process.describe
        # logger.debug(f' -> Resources Allocation: {allocation}')
