...
def get_rate_of_improvement(cost, prev_cost,second_prev_cost):
    ...
    return acceleration
def run_experiment(params, steps, n_rsv_qubits, n_ctrl_qubits,  K_coeffs,hZ, trotter_steps, static, gate, gate_name, test_size, training_size,opt_lr= None,L = [], key=0):
    bath = False
    dqfim_data = None
    ...
    qrc = QuantumReservoirGate(n_rsv_qubits=n_rsv_qubits, n_ctrl_qubits=n_ctrl_qubits, K_coeffs=K_coeffs,hZ=hZ,trotter_steps=trotter_steps, static=static)
    ...
    def circuit(params, input_state):
        ...
        for i in range(trotter_steps):
            ...
            if trotter_steps==1:
                qrc.set_gate_params(x_coeff=x_coeff,y_coeff=y_coeff, J_coeffs=J_coeffs)
            else:
                step = len(qrc.rsv_qubits)*len(qrc.ctrl_qubits)
                qrc.set_gate_params(x_coeff=x_coeff,y_coeff=y_coeff,J_coeffs=J_coeffs[i*step:(i+1)*step])
            ...
            for trainable_params_dict in df[fixed_params_dict].keys():
                if trainable_params_dict == trainable_param_dict_key:
                    ...
                    if 'qfim_eigvals' in results:
                        ...
                        fixed_params = {'K_coeffs':results['K_coeffs'], 'hZ':results['hZ'], 'E_0':results['E_0'], 'K_0':results['K_0']}
                        try:
...
# --------------------------------------------------------------------------------------------
    ...
    folder = f'./param_initialization_final/digital_results_fixed_hZ/Nc_{N_ctrl}/'
    gates_random = []
    for i in range(50):
    ...
    fixed_param_keys = [f'fixed_params{fp_idx}',f'fixed_params{fp_idx}',f'fixed_params{fp_idx}']
    trainable_param_keys = ['test76','test0','test63', 'test40','test45','test3']
    ...
    list_of_trainable_sets = {"1": trainable_param_keys,
                             }
    ...
    for gate_idx,gate in enumerate(all_gates):
        for fixed_param_name, (sample_range_label, trainable_param_keys) in zip(fixed_param_keys,list_of_trainable_sets.items()):
            for test_key in trainable_param_keys:
                ...
                folder_gate = folder + '/reservoirs_' + str(N_r) + '/trotter_step_' + str(trotter_steps) + f'/{base_state}'+  '/' + f"/{training_size}_training_states_random/" + str(fixed_param_name)+  '/'+ test_key_dict+f'/' + gate.name + '/'
                ...
                state = 'GHZ'
                Kfactor = '1xK'
                qfim_base_path = f'/Users/so714f/Documents/offline/qrc/QFIM_traced_final_results/gate_model_hZ_DQFIM/Nc_{N_ctrl}/L_{num_L}/{Kfactor}/'
                ...
                eigvals, fixed_params,params,qfim,L,entropies = get_qfim_eigvals(qfim_file_path, fixed_param_name, test_key)
                K_coeffs = fixed_params['K_coeffs']
                hZ = fixed_params['hZ']
                E_0 = fixed_params['E_0']
                K_0 = fixed_params['K_0']
                L = []
            ...
            # A, b, opt_lr,first_grad = optimize_traingset(gate,N_ctrl, N_r,trotter_steps, params, K_coeffs, training_size,5,set_key)
                ...
                data = run_experiment(params = params,steps =  steps,n_rsv_qubits= N_r,n_ctrl_qubits= N_ctrl,K_coeffs= K_coeffs,hZ=hZ,trotter_steps= trotter_steps,static= static,gate=gate,gate_name= gate.name,test_size= test_size,training_size= training_size,opt_lr=opt_lr, L=L,key=set_key)
