...
def get_groupwise_lr_trees_new(
    ...
    *,
    ...
    def eta_ceiling(tgt, g):
        return jnp.minimum(max_lr, tgt / (jnp.linalg.norm(g) + eps))

    def _eta(raw_target, block_grad):
        return raw_target / (jnp.linalg.norm(block_grad) + eps)

    
    
    raw_eta_tau = _eta(tgt_tau, g_tau)
    eta_tau = jnp.minimum(max_lr, raw_eta_tau)
    eta_tau2 = eta_ceiling(tgt_tau, g_tau)
    assert eta_tau == eta_tau2

    # raw_eta_h   = _eta(tgt_h,   g_h)
    # eta_h   = jnp.minimum(max_lr, raw_eta_h)

    # raw_eta_J   = _eta(tgt_J,   g_J)
    # eta_J   = jnp.minimum(max_lr, raw_eta_J)

    
    eta_h   = eta_ceiling(tgt_h,   g_h)
    eta_J   = eta_ceiling(tgt_J,   g_J)
    ...
    r_tau, r_h, r_J = med_mad(g_tau), med_mad(g_h), med_mad(g_J)

    if scale_by_num_train:
        if   num_train >= 20: factor = NC / 8
        elif num_train >= 11: factor = NC / 4
        else:                 factor = NC / 2
        r_tau *= factor; r_h *= factor; r_J *= factor
    # ──────────────────────────────────────────────────────────────
    # per-param learning rates
    # ──────────────────────────────────────────────────────────────
    def per_param_lr(η, r, g):
        raw = η * r / (g + r + eps)
        return jnp.clip(raw, 1e-6, max_lr)
    lr_tau = per_param_lr(eta_tau, r_tau, jnp.abs(g_tau))
    lr_h   = per_param_lr(eta_h,   r_h,   jnp.abs(g_h))
    lr_J   = per_param_lr(eta_J,   r_J,   jnp.abs(g_J))
    ...
    if debug:
        print("\n── get_groupwise_lr_trees_new DEBUG ──")
        print(f"max_lr                = {max_lr:.3e}")
        print(f"scale_by_num_train    = {scale_by_num_train}")
        print(f"per_block_target_upd  = {per_block_target_update}")
        if per_block_target_update:
            print("target_update ceilings:")
            print(f"  τ : raw={tgt_tau:.3e}  clipped={eta_tau:.3e}")
            print(f"  h : raw={tgt_h :.3e}  clipped={eta_h :.3e}")
            print(f"  J : raw={tgt_J :.3e}  clipped={eta_J :.3e}")
        else:
            raw_global = float(target_update / (jnp.linalg.norm(grads) + eps))
            print(f"global target_update = {target_update:.3e}")
            print(f"η_max (raw → clipped) = {raw_global:.3e} → {eta_tau:.3e}")

        print("r (median+MAD) values:")
        print(f"  r_tau={r_tau:.3e}, r_h={r_h:.3e}, r_J={r_J:.3e}")
        print("final lr stats:")
        for name, lr in [("τ", lr_tau), ("h", lr_h), ("J", lr_J)]:
            print(f"  {name}-group: min={jnp.min(lr):.3e}, "
                  f"max={jnp.max(lr):.3e}, mean={jnp.mean(lr):.3e}")
        print("────────────────────────────────────────\n")
